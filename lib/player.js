...
class Player{
  ...
  placeShips(player){
    ...
    let boardCopy;
    //Add ship to top left corner of board
    for(let shipIdx = 0; shipIdx < this.ships.length; ++shipIdx ){
      ...
      //Get coordinates for positions of ship
      clearConsole();
      console.log("Now placing ship " + (shipIdx  + 1) + ": [" + ship.symbol + "]")
  ...
  addShip(ship, board, headCoords, tailCoords){
    //Add the ship in the given orientation to game board
  ...
  shipOptions(coords, ship, board, boardCopy){
    ...
    let left = true;
    let right = true;
    //Check which directions the ship can be placed
    for(let i = 0; i < ship.length; ++i){
    ...
    let j;
    let valid = [];
    //Display possible ship orientations
    if(right){
  ...
  getCoords(player, ship, boardCopy, str, board){
    //If not valid gets coordinates again
  ...
  checkInput(input, ship, boardCopy, board){
    //Checks length of input, valid first character, valid second number
    if(input.length < 2){
      return false;
    }
  ...
  getGuess(){
    //Gets valid and unguessed tiles
    let guess = this.checkGuess(readlineSync.question("Enter coordinates to guess: ").toLowerCase());

    while(!guess){
      guess = this.checkGuess(readlineSync.question("Already guess or out of bounds! Try again: ").toLowerCase());
    }
  ...
  markBoard(coords, board, ships){
    ...
    //Change next players board and return status of guess
    if(tile === false){
      board.grid[coords[0]][coords[1]] = true;
      return "was a miss!";
    }else{
      tile.hit = true;
